...
def list_parcels(client, days, active_only=False, courier=None, debug=False):
    ...
    try:
        if active_only:
            ...
            logger.info(f"Retrieving parcel history from {start_date.date()} to {end_date.date()}...")
...
def main():
    ...
    parser.add_argument("--days", type=int, default=30, 
                        help="Number of days in the past to check (default: 30)")
    ...
    list_parser.add_argument("--active", "-a", action="store_true", 
                           help="Show only active (not picked up) parcels")
    ...
    export_parser.add_argument("--active", "-a", action="store_true", 
                             help="Export only active (not picked up) parcels")
    ...
    try:
        ...
        if args.command == "list":
            ...
            if parcels:
                if args.format == "csv":
                    output_file = args.output or f"parcels_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
                    client.export_to_csv(parcels, output_file)
                    logger.info(f"Exported {len(parcels)} parcels to {output_file}")
                else:  # json
                    output_file = args.output or f"parcels_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
